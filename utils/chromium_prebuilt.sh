...
PRODUCT_COPY_FILES += \\
    ...
    \$(shell mkdir -p out/target/product/__DEVICE__/system/app/webview/lib/$(get_build_var TARGET_ARCH)/)
    \$(shell cp -r \$(LOCAL_PATH)/app/webview/lib/arm/libwebviewchromium.so out/target/product/__DEVICE__/system/app/webview/lib/$(get_build_var TARGET_ARCH)/libwebviewchromium.so)
