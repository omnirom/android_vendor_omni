...
PRODUCT_COPY_FILES += \\
    ...
    \$(shell mkdir -p out/target/product/__DEVICE__/system/app/webview/lib/$ARCH/)
    \$(shell cp -r \$(LOCAL_PATH)/app/webview/lib/$ARCH/libwebviewchromium.so out/target/product/__DEVICE__/system/app/webview/lib/$ARCH/libwebviewchromium.so)
