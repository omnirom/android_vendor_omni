...
Additional OmniROM functions:
 - breakfast:            Setup the environment, similar to lunch,
                         but will only list supported devices
 - brunch:               Uses breakfast to setup environment for
                         selected device and builds using 'mka bacon'
 - fixup_common_out_dir: Links the appropriate directory to out/target/common
                         every time lunch is run
